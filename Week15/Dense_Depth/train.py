...
def main():
    ...
    if args.start_epoch==0:
        pretrained_model2=False
        original_model2 = net_mask.drn_d_22(pretrained=pretrained_model2)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        train_log_df=train(train_loader, model, model2, optimizer, epoch)
        train_log_df.to_csv('/content/drive/My Drive/train_log.csv',mode='a',header=None)
        #torch.save(model.state_dict(), '/content/drive/My Drive/model_resnet_new')
        torch.save(model2.state_dict(), '/content/drive/My Drive/mask_save')
...
def train(train_loader, model, model2, optimizer, epoch):
    ...
    for i, sample_batched in enumerate(train_loader):
        ...
        ones = torch.ones(depth_.size(0), 1, output.size(2),output.size(3)).float().cuda()
        ones = torch.autograd.Variable(ones)
        ...
        #print(mask.shape,loss_sparse.shape,image.shape)
        #print('mask mean:',loss_sparse, loss_sparse.data)
        losses.update(loss_sparse.data, image.size(0))
        loss.backward()
        optimizer.step()
        ...
        train_log_list.append([epoch, i, len(train_loader),batch_time.val, batch_time.sum, losses.val.item(), losses.avg.item()])

    print('errors: ', averageError)
    train_log_df=pd.DataFrame(train_log_list)
    #test_log_df.to_csv('test_log.csv')
    return train_log_df
